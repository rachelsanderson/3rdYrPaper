...
\subsection{Estimation with the linked datasets}

Describe what I actually do??
...
% fix this or add to discussion
Based on these results, ABE Multi seems to perform well if the AHL estimator will be used.  This is because  ABE Multi is very likely to include the true match among the matches.  It is also easier in terms of computation, because it does not require linear sum assignment programs or thresholds to determine which record pairs should be designated as matches.  Furthermore, we know the efficiency gains from obtaining estimated probabilities are minor for $N$ large. 
