...
Merging data is a necessary first step for projects that use multiple data sources with overlapping units.  When the datasets are large, automated record linkage procedures offer the least-cost solution for performing a one-to-one merge.  These procedures come with the caveat that in the presence of imperfect identifiers, they may produce false matches, or exclude true matches.  One solution is to include multiple possible matches.  

However, which automated record linkage procedure to use 
