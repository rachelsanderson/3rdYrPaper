...
% \setstretch{1.25}
\doublespacing
\title{\singlespacing Analyzing linked data}
...
When analyzing multiple data sources with overlapping units, automated record linkage procedures offer a low cost solution for merging data.  
Instead of clerical review, they allow the researcher to specify a set of matching variables, and a decision rule, to obtain a matched dataset in a short period of time.  [[talk here]]

Increased availability and use of large administrative datasets in the social sciences has sparked interest in studying how these methods impact data quality.  This is particularly salient issue for economic historians, who work with data that lack social sefurity numbers. For example, if data are prone to typographical error, duplication, and mis-reporting, it is important to understand how automated linking methods may exacerbate sample selection.   clerical review is tedious and costly, and so automated methods reduce human computational burden (and potentially bias).  These procedures come with the caveat that in the presence of imperfect identifiers, they may produce false matches, or exclude true matches.  

The tradeoffs among these methods are well documented 

One solution is to improving the match rate is to include multiple possible matches.  Another is to esitmate the probability of the match, and correct for potnetial biases introduced by the matching process.  


Different procedures give different outputs -- for example, some produce multiple matches for each observation; others allow the researcher to estimate the probability that any given pair of observations refers to a match.  Consequently, different estimation methods are available depending on which matching procedure is used.  This paper compares the performance of existing estimation methods for matched data, and extends the methods described in \cite{ahl2019} to incorporate probabilities and correct for a nonparametric step. 

The tradeoffs between automated record linkage procedures in terms of matched dataset quality has been studied.  While match quality is important, choosing which record linkage procedure to use should also depend on which estimation methods will be used in subsequent analysis.   
...
To fix ideas, consider the work of \cite{aizer2016}, who seek to estimate the effect of providing cash transfers to single mothers on the life expectancy of their children.  The $x$-datafile consists of mothers' welfare program applications, where $x_i$ includes a binary variable equal to 1 if person $i$'s mother received a cash transfer, and other demographic variables.  The $y$-datafile is a universal database of death records, which includes observations of $y_j$, person $j$'s age at death.  Both of the $x$- and $y$-datafiles also contain $w_i$ and $w_j$, which include the observation's first and last names and year of birth, so that common names and typographical error are likely to complicate even the most conservative assignment of $\varphi$. 

In light of these challenges, researchers have developed a variety of automated record linkage methods for constructing $\varphi$, some of which are described in Section X.  Such procedures result in a dataset $\{x_i, \{y_{i\ell}\}_{\ell=1}^{L_i}\}_{i=1}^N$, such that a single observation $x_i$ may be linked to $L_i$ possible values of $y$.   Note that $N_x \geq N$, which holds with equality if and only if every observation $x_i$ is assigned a match.  Additionally, some record linkage procedures can be used to estimate the probability that a given $y_{i\ell}$ is the true match, denoted $\hat{\pi}_{i\ell}$, such that $\sum_{\ell=1}^{L_i} \hat{\pi}_{i\ell} =1 $ for all $i$. 

As there is already a well-established literature that compares the performance of different record linkage methods, this paper does not compare matching methods in terms of how their outputs can be used during estimation.  As will be discussed in the later sections, different record linkage procedures either produce one or multiple matches per observation, and sometimes output probabilities.  Hence, the goal is to study how estimation proceeds in each of these cases, and compare their relative performance.  The hope of this exercise is to guide which combination of matching method and estimation procedure should be used in practice. 

For now, we take the matched dataset as given, and discuss some of the methods for analyzing linked data. ß
...
\subsection{Applying argument to OLS}
...
\begin{equation} a_1 y_{i1} + a_2 y_{i2} - \kappa = x_i'\beta + \varepsion_i \end{equation}
...
\begin{align*}
\hat{\beta}_{1} &= \frac{1}{N}\sum_{i=1}^N \frac{x_i y_{i1}}{\pi} - \frac{1-\pi}{\pi}\kappa \\
 \hat{\beta}_{2} &= \frac{1}{N}\sum_{i=1}^N \frac{x_i y_{i2}}{1-\pi} - \frac{\pi}{1-\pi}\kappa 
 \end{align*}
 ...
 \end{equation}
and we can repeat the exercise in the previous sections, comparing variance and bias for misspecified beliefs $\hat{\pi}$ and different parameter combinations.  The choice of the weights $d^*$ that give the optimal $\hat{\beta}^*$ is now complicated by the fact that it depends on the second moments of $X_i$, so this is best performed by simulation. 



\section{Numerical Example}
Now I study how these methods work in practice.   This section introduces a numerical example, that illustrates what I do in the Monte Carlo below. 
