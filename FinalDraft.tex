...
\subsection{Proofs}
\noindent \textit{\textbf{Lemma 1.}} Consider constructing a linear unbiased estimator of $\mu$ that places positive weight on both observations $X_1$ and $X_2$, 
\begin{equation}
...
\end{align*}
which completes the proof.  Recall that $a_1$ is only pinned down by minimizing the variance of $\hat{\mu}^{(\ell)}$. 
