...
\begin{abstract}
\singlespacing
\noindent This paper compares methods for estimating linear regression models when the variables of interest are observed in distinct datasets with ambiguous identifiers.  In this setup, the researcher must attempt to identify which observations refer to the same individual prior to estimation, so that random errors in matching result in biased estimators.  \cite{sw1993} suggest correcting for the bias by incorporating knowledge about the probability that a given match is correct, however \citet*{ahl2019} propose an unbiased estimator that only requires that the true match is included among an observation's possible matches.  This paper establishes conditions under which these methods are equivalent, and compares their performance using matched data that is linked with different record linkage procedures.  The results suggest that the AHL estimator offers unbiasedness under weaker assumptions that the SW estimator, without sacrificing efficiency, and suggests further areas for study. \end{abstract} 
...
The $y$-datafile includes all 1,000 values of $y_i$ from the ground truth data, along with the original identifiers $w_i$.  The aim of this construction is to make it likely that some observations in the $x$-datafile will be linkejd to multiple values of $y$.  The next section describes the record linkage methods used to link the $x$- and $y$-datafiles. 
...
\addlinespace
\textbf{Example 2.}  The identifiers in the simulated data do not include non-alphabetic characters, but do include misspelled names, so I pre-process all of the first and last names using the New York State Identification and Intelligence (NYSIIS) phonetic algorithm, and include these phonetically-spelled names among the matching variables.  The other matching variable is birth year, which does not require standardization.   Other popular phonetic algorithms include Soundex\footnote{This is the phonetic algorithm used in \citet*{aizer2016} from Example 1.} and Metafone; however, I assume that the NYSIIS algorithm performs sufficiently well for the purposes of my analysis, given that the names I use are selected from among the most common names assigned at birth in the present-day United States.  
\addlinespace
...
The probabilistic matching methods, referred to as PRL Single and PRL Multi, are implemented using the fastLink package in \texttt{R} created by \cite{enamorado2019}.  Their methods are based upon the canonical work of \cite{fellegi69}, which I now review. 
...
Since membership to $M$ or $U$ is not observed, \cite{larsen_rubin_2001} suggest using the expectation-maximization (EM) algorithm from \cite{em} to simultaneously estimate the parameters in (\ref{mm}) and classify record pairs as matches or non-matches.  Crucially, there is no restriction that the posterior match probabilities in (30) sum to 1 for a fixed observation $i$.  Obtaining unique matches requires using a linear sum assignment program that maximizes the sum of the posterior matching probabilities subject to the constraint that no observations in either datafile can be matched multiple times. 

The fastLink package by \cite{enamorado2019} estimates the posterior match probability for each pair of observations in the sample, as outputted by the EM algorithm.  Their method allows the user to specify a lower bound for the posterior probability of a match that will be accepted; I set this equal to 0.7 because the default value of 0.85 does not result in any matches.   The fastLink algorithm also allows the user to specify whether unique matches are desired, and, if so, returns the set of matches that solves the linear sum assignment program assigned above.  This is how I obtain PRL Single.  To obtain PRL Multi, I accept all matches with posterior match probabilities greater than 0.7, and I normalize those probabilities to obtain $\pi_{i\ell}$. 

\section{Monte Carlo Results}

Following the data generating process described in Section 5.1, I generate 1,000 $x$- and $y$-dataset pairs, such that each of the 500 observations in the $x$-datafile has a unique, true match in the $y$-dataset, but the identifiers in the $y$-dataset may repeat, and the identifiers in the $x$-dataset contain random errors.  I then match each dataset pair a total of four times, using two deterministic matching methods, ABE Single and ABE Multi, and two probabilistic record linkage methods, PRL Single and PRL Multi.   ABE Single and PRL Single require that each $x_i$ is matched to at most one $y_j$, and that each $y_j$ is matched to at most one $x_{i}$.  ABE Multi and PRL Multi enforce no such requirement, so that a single observation in the $x$- datafile may be matched to multiple observations in the $y$-datafile, and a single observation in the $y$-datafile may be a match for multiple values of $x_i$. 
...
The first column of Table \ref{match_rate} reports the proportion of observations in the $x$-datafile that are linked to at least one observation in the $y$-datafile, averaged across the Monte Carlo replications.  These rates range between 71 and 79 percent across methods, however Figure \ref{match_hist} shows that ABE Multi consistently matches more observations than any other procedure.  Additionally, PRL Single and PRL Multi have the same match rate, which suggests that allowing for multiple matches adds additional matches per observations, as opposed to matching new individuals.  This is an artifact of the fastLink algorithm, because the posterior probability threshold for designating a match is the same for both PRL Single and PRL Multi.  By contrast, ABE Multi seems to increase the match rate relative to ABE Single by matching  observations that are discarded by ABE Single. 

The second column of Table \ref{match_rate} displays the average number of links assigned by each matching procedure.  For ABE Multi and PRL Multi, each $(i, j)$ combination counts as a distinct link, so that an observation with $L$ linked outcomes counts as $L$ matches.  To explore whether these numbers are driven primarily by linking the observations to many matches, or by linking more observations to possibly multiple, but fewer, matches, I report the average number of links per observation in Table \ref{multi_L}.  On average, ABE Multi seems to assign larger $L_i$ per observation:  PRL Multi assigns a unique match to 59 percent of observations, and two matches to 33 percent of observations, compared to ABE Multi which assigns, on average, one, two, and three matches to 52, 35, and 11 percent of observations, respectively.  Both algorithms tend not to assign more than four matches for many observations.  Note that these probabilities do not sum to one, because they reflect the average of $P(L_i = \ell)$ across 1,000 observations, and not the average \textit{distribution} of $L_i$ across datasets. 

Although ABE Single and PRL Single match a similar number of observations on average, ABE Single seems to produce higher quality matches as measured by its low Type I error (0.03 compared to 0.11 for PRL Single).  This comes at the cost of a high Type II error rate: ABE Single fails to match about 26 percent of possible links on average, compared to 15 percent for PRL Single.  As discussed in Section 5, about 6 percent of the Type II error can be attributed to the blocking structure imposed by the ABE algorithm; however, this cannot fully explain the gap between the two methods.  

% TO DO : FIX TYPE II ERROR RATE FOR TE MULTIPLE MATCH METHODS! cannot be correct

The last column in Table \ref{match_rate} reports the average probability that the set of matches $\{y_{i\ell}\}_{\ell=1}^{L_i}$ contains the true match.  This is an important metric to study, because the estimation methods used in this paper assume that this probability is equal to 1.  Notably, this assumption is most likely to fail for PRL Single (it finds the correct match only 89 percent of the time), but allowing for multiple matches in PRL Multi improves this probability significantly.   ABE Single and ABE Multi perform very well on this metric, including the correct match 97 and 99 percent of the time.  Breaking down this probability by the number of matches $L_i$ for ABE Multi and PRL Multi in Table \ref{multi_L} shows that allowing for multiple matches increases the probability that the true match is included in the sample.  
...
Based on Figure \ref{ahl_sw}, the AHL estimator appears to perform better than the SW estimator for both datasets, because the SW estimator for $\beta_0$ is not centered around the true value, and the distribution of the SW estimator has fatter tails than that of the AHL estimator for all parameter values.  The AHL estimator also performs better than the SW estimator as measured by the median absolute deviation across the 1,000 replications, and for both datasets (see Table\ref{mad}).   Furthermore, the AHL estimator performs better than the OLS estimator that uses only those observations assigned $L_i =1$ by the ABE Multi and PRL Multi methods, whereas the SW estimator performs worse.  The AHL and SW estimators cannot be compared using the datasets matched by ABE Single or PRL Single, because they are both equal to the Naive OLS estimator when $L_i = 1$.  
...
Table \ref{mad} also compares the AHL and SW estimators to the OLS estimator that uses only the true matches appearing in the matched datasets (OLS True).  As expected, no estimator performs as OLS True, however both AHL and SW do not perform significantly worse in terms of median absolute deviations. 
...
%Here I try to derive the variance of the AHL estimator based on the difference between conditional moments, but I ran out of time
%\begin{align*}
