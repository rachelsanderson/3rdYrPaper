...
When the assumptions of $\hat{\beta}_{SW}$ fail, one solution is to use methods from \cite{ahl2019}, which is unbiased under Assumptions (i) and (ii) from Section 2.  Recall that these are (i) the true match of $x_i$ is included among the matches $\{y_{i\ell}\}_{\ell=1}^{L_i}$ for all $i$, and (ii) $x_i$ and $y_i$ are random samples conditional on the matching variables.  Assumption (ii) allows $x$ and $y$ to be correlated with errors in matching, so long as that dependence is fully captured by $w$; hence the AHL estimator would be valid in the \cite{nq2015} if ethnicity is included among the matching variables.  
...
I begin with a simplified version of the AHL GMM framework, based on the observation that estimating $E[m(y_i, x_i; \theta)]$ is the same as estimating the mean... 
