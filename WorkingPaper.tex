...
which is $$d^*= \frac{\Var{\hat{\mu}_2} - \text{Cov}(\hat{\mu}_1, \hat{\mu}_2)}{\Var{\hat{\mu}_1} + \Var{\hat{\mu}_2} - 2 \text{Cov}(\hat{\mu}_1, \hat{\mu}_2)}$$ 
Observe that,
...
\text{Cov}(\hat{\mu}_1, \hat{\mu}_2) &= \frac{\text{Cov}(X_1,X_2)}{\pi(1-\pi)} = \frac{1}{\pi(1-\pi)}\left((1-\pi^2 - (1-\pi)^2)\mu\kappa - \pi(1-\pi)(\mu^2 + \kappa^2)\right)
\end{align*}
...
Pick parameter values better...

Hence the optimal estimator $\hat{\mu} = d^* \hat{\mu}_1 + (1-d^*) \hat{\mu}_2$ depends on $\sigma^2, \omega^2, \mu,$ and $\kappa$.  To see how these change, Figure \ref{dStar} demonstrates.   All estimators are unbiased; the plotted value just shows the optimal ratio of the two unbiased estimators to achieve the minimum variance unbiased estimator.  When the variance of both the correct and incorrect distributions are the same, then $d^*$ weights according to the probability of being correct.  When teh incorrect distribution has high variance, it becomes costlier to weight the incorrect distribution. We see that when the variance is the same, the mixture
...
\caption{Par values $(\sigma^2, \omega^2, \mu, \kappa) =  ( 1, 20, 0, 1 )$}
...
Observe that AHL variance is constant becase var (x1) + var (x2) + 2*cov(x1,x2) does not depend on pi. 
