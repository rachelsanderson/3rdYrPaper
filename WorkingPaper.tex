...
where $x_i$ and $y_i$ are recorded in different datasets, so that they must be linked using auxiliary variables that are contained in both data sources.  

Formally, the data consist of the $x$-datafile with observations $\{x_i, w_i\}_{i=1}^{N_x}$, and the $y$-datafile with observations $\{y_j, w_j\}_{j=1}^{N_y}$.  We assume that $N_y\geq N_x$, and that that each observation $x_i$ has a true match $y_i$ contained in the $y$-datafile, however the index $j$ that corresponds with this match is unknown.  Observations $y_j$ may or may not have a match in the $x$ dataset.  If they do, they are generated according to the model \ref{model}, but if not then we don't know!
 



the matching step consists of estimating a function, \begin{equation} \varphi: \{1,\dots, N_x\} \to \{1,\dots, N_y\} \cup \varnothing \end{equation} where $\varphi(i) = j$ if individual $i$ in the $x$-datafile and individual $j$ in the $y$-datafile refer to the same entity, and $\varphi(i) = \varnothing$ if $i$ does not have a match in $y$-datafile.  Note that if $w$ identifies individuals uniquely and without error, then $\varphi(i) = j$ if and only if $w_i = w_j$, and $\varphi(i) = \varnothing$ otherwise.  However, if $w$ is not unique or recorded with error, then $\varphi$ needs to be estimated, and inference about $\beta$ may need to be adjusted accordingly.   
