...
\newpage
\section{Solving the multiple matches problem}
...
which is \begin{equation} d^*= \frac{\Var{\hat{\mu}_2} - \text{Cov}(\hat{\mu}_1, \hat{\mu}_2)}{\Var{\hat{\mu}_1} + \Var{\hat{\mu}_2} - 2 \text{Cov}(\hat{\mu}_1, \hat{\mu}_2)} \label{dOpt}\end{equation}
Observe that,
...
\end{align}
where $\Var{X_1}$ and $\Var{X_2}$ are calculated using the law of total variance, using the random variable $D = 1$ if $X_1$ is drawn from the correct distribution (and $X_2$ is drawn from the incorrect distribution), and $D=0$ otherwise:
...
\begin{equation} \hat{\mu}(d^*) = d^* \hat{\mu}_1 + (1-d^*) \hat{\mu}_2 \label{muOpt}
\end{equation}
where $d^*$ is defined as in (\ref{dStar}), and is a function of $\sigma^2, \omega^2, \mu,$ and $\kappa$.  This dependence can be seen in Figure \ref{dStar}, which plots the optimal $d^*$ for different values of $\pi$ and different parameter combinations.  

First, observe that when $\pi=0.5$, $\Var{\mu_1}=\Var{\mu_2}$ so that $d^*=\frac{1}{2}$, regardless of the other parameter values.  When the variance of both the correct and incorrect distributions are the same (i.e., $\sigma^2 = \omega^2$, as in the red line), then $\Var{X_1}=\Var{X_2}$, and differences in $d^*$ reflect only changes in $\pi$.   As shown in (\ref{dStar}), the optimal $d^*$ puts more weight on the estimator with lower variance; which is equivalent to weighting the estimator based on the observation that is more likely to come from the low variance distribution.  The purple line represents the case where $\kappa$ may be estimated, and so uncertainty about the incorrect distribution is high.  In this case, the optimal estimator puts low weight on the observation that is more likely drawn from the incorrect distribution.   When the correct distribution has larger variance, we are more likely to equally weight the estimators.  
...
Next, we consider the case where $\pi$ is unknown, but the econometrician has beliefs $\hat{\pi}$ about the probability that $X_1$ is drawn from the correct distribution.  In the context of estimation using matched data, $\pi$ refers to the probability that a record pair refers to a match, and $\hat{\pi}$ may be estimated using a probabilistic record linkage procedure, or reflect the researcher's prior knowledge about the matching application\footnote{For example, the researcher may believe that ``Alicia" is more likely than ``Alex" to refer to the true match of an individual named ``Ali", and so her beliefs $\hat{\pi}$ would reflect this.}.

In such a scenario, it is interesting to ask how the bias and variance change for the estimator in (\ref{muOpt}), if $\hat{\mu}_1, \hat{\mu_2}$ and $d^*$ are solved using $\hat{\pi}$ in place of the true $\pi$.   

For example, if $\pi=0.6$ and $(\mu, \sigma^2, \kappa, \omega^2) = (0, 1, 1, 2)$, but the researcher believes $\hat{\pi}=0.9$, then she computes
\begin{gather*}
\hat{\mu}_1 = \frac{10}{9} X_1 - \frac{1}{9} \\
\hat{\mu}_2 = 10 X_2 - 9
\end{gather*}
each of which are biased, because $E[\hat{\mu}_1] = \frac{1}{3}$ and $E[\hat{\mu}_2] = -3$.  Similarly, using $\hat{\pi}$ to calculate $\Var{\hat{\mu}_1},\ \Var{\hat{\mu}_2}$, and Cov$(\hat{\mu}_1, \hat{\mu}_2)$, results in $d^* = 0.987$.  Letting $$\hat{\mu}=d^* \hat{\mu}_1 + (1-d^*)\hat{\mu}_2 $$ 
we have $E[\hat{\mu}] = 0.292 \neq \mu$ and $\Var{\hat{\mu}} = 1.94$ for one observation.  

Compare this to using estimating $\hat{\mu}$ by weighting the observations equally,
\begin{gather*}
\hat{\mu} = X_1 + X_2 - 1
\end{gather*}
Now, $E[\hat{\mu}] =  0$ and $\Var{\hat{\mu}} = 3$ for one observation.  Hence we can calculate $N$ such that the MSE is equal for the estimator based on $\hat{\pi}$ and the estimator that gives the observations equal weights by solving,

$$0.292^2 + \frac{1.94}{N} = \frac{3}{N} \implies N = 12.43$$ 
In general we can solve for $|\hat{\pi}-\pi}|$ and parameter values and a fixed sample size $N$ whether you are better off using beliefs about the $X$ or being agnostic about which is correct!


The ratio of the MSE of this agnostic estimator, which treats each observation as equally likely to be correct is   %% WHY NOT JUST MULTIPLY BY A SUPER SMALL CONSTANT? 

To test this, I compute the ratio of MSE of the optimal estimator in (\ref{muOpt}) where $d^*$ is calculated using $\hat{\pi}$ instead of $\pi$, compared to the AHL estimator (which is the same as acting as if $\hat{\pi}=0.5$. 

The ratio is presented in the Tables below for the same parameter combinations as before.  
