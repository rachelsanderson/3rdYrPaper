...
Observing (\ref{bias}), \cite{sw1993} proposed estimating $\hat{B}$ to correct for the bias of $\hat{\beta}_N$.   To reduce the computational burden of constructing $\hat{B}$, they suggest using the first and second highest elements of the vector $q_i$ and their corresponding values $y_{ij_1}$ and $y_{ij_2}$ to compute $\hat{B}_i^{TR} = (q_{ij_1} - 1) y_{ij_1} + q_{ij_2} y_{ij_2}$, and then calculating
% write what are z_j
\begin{equation} \hat{\beta}_{SW} = \hat{\beta}_N - (X'X)^{-1} X' \hat{B}^{TR} \label{sw}\end{equation}
Although $\hat{\beta}_{SW}$ can incorporate any number of elements of $q_i$, \cite{sw1993} note that if the probability is high that the best candidate link is the true link, then the truncation results in a very small bias. 
 
The downside of this method is that it requires knowledge of $q_{ij}$, as well as the second most likely value of $y$ for each observation of $x$.  This information is not typically available when using deterministic matching procedures such as those developed by \cite{abe2012}.   However, even if  estimates of $q_{ij}$ are available (as is the case when probabilistic record linkage is used), the bias may persist if $\hat{q_i}$ is correlated with $y$, as this would introduce endogeneity.  
To see this explicitly, note that $\hat{\beta}_{SW}$ is the same as applying OLS to the model:
\begin{equation}
2  y_{ij_1} - (\widehat{q_{ij_1}}y_{ij_1} + \widehat{q_{ij_2}} y_{jj_2})= x_i'\beta + \varepsilon_i
\end{equation}
